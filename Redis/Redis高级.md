...
![image-20200715095057380](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200715095057380.png)
...
![image-20200715094731624](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200715094731624.png)
...
![image-20200715101141839](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200715101141839.png)
...
![image-20200715102104487](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200715102104487.png)
...
![image-20200715102400058](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200715102400058.png)
...
![image-20200715102514355](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200715102514355.png)
...
![image-20200715102749046](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200715102749046.png)
...
![image-20200717092539841](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717092539841.png)

2）放弃事务

![image-20200717092930121](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717092930121.png)

3）事务异常情况，前面说了Redis中的事务是没有回滚操作的，出现异常分为两种情况

- 添加指令时异常（在multi后，exec前抛出异常），那么事务里面的指令会全部不执行

![image-20200717093035561](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717093035561.png)

- 执行指令时异常（在exec后抛出异常），此时这种情况会对事务中的可正常执行的指令全部正常执行，异常指令返回异常。

![image-20200717101653435](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717101653435.png)
...
![image-20200717152654862](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717152654862.png)



2）不可重复读：在基于锁的并行控制方法中，如果在执行select时不添加读锁，就会发生不可重复读问题。在多版本并行控制机制中，当一个遇到提交冲突的事务需要回退但却被释放时，会发生不可重复读问题。

![image-20200717153903452](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717153903452.png)
...
![image-20200717165830644](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717165830644.png)
...
- 重写条件
	- 系统载入时或者上次重写完毕时，Redis会记录此时AOF大小，设为base_size,如果Redis的AOF当前大小>= base_size +base_size*100% (默认)且当前大小
       >=64mb(默认)的情况下，Redis会对AOF进行重写
```
...
![image-20200717193502303](E:\Hadoop生态\大数据总结\Redis\Redis高级.assets\image-20200717193502303.png)
...
![image-20200719173925724](.\Redis高级.assets\image-20200719173925724.png)
...
```txt
- Redis 集群实现了对Redis的水平扩容，即启动N个redis节点，将整个数据库分布存储在这N个节点中，每个节点存储总   数据的1/N。
...
![image-20200719204441385](.\Redis高级.assets\image-20200719204441385.png)

![image-20200719204425184](.\Redis高级.assets\image-20200719204425184.png)
...
![image-20200719211110498](.\Redis高级.assets\image-20200719211110498.png)
...
![image-20200719211258632](.\Redis高级.assets\image-20200719211258632.png)
...
![image-20200719212444219](.\Redis高级.assets\image-20200719212444219.png)
...
![image-20200719211258632](.\Redis高级.assets\image-20200719211450780.png)
...
```txt
- redis.conf中的参数  cluster-require-full-coverage 默认是yes ，如果选no那么即使某一部分的slot完全下线(包括从机)，集群也会继续以现存的数据提供服务。 
```
...
![image-20200720085339632](.\Redis高级.assets\image-20200720085339632.png)
...
![image-20200720085312064](.\Redis高级.assets\image-20200720085312064.png)
...
![image-20200720085232585](.\Redis高级.assets\image-20200720085232585.png)
